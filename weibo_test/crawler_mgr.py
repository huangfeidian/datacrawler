...
class crawler_mgr:
    ...
    def set_worker_list(self,wk_list_file_name):
        header={
        ...
        for name in wk_name_list:
            ...
            new_worker.set_ss_config(self.proxylist[i%proxy_len],header,self.cookie_dict[name]);
            self.worker_list.append(new_worker);
    def run(self):
        threads=[];
        for i in self.worker_list:
            temp_thread=threading.Thread(target=i.run,args=());
